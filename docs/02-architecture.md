...
> **编辑边界**：可调整模块拆分细节，但读写路径策略（读 SDK / 写云函数）为已锁定决策。
...
└──────────┬─────────────┘        └──────────────┬───────────┘
           │ 读: CloudBase JS SDK 直连           │ 读: CloudBase SDK
           │ 写: 调用云函数                       │ 写: 调用云函数
           ▼                                     ▼
┌────────────────────────────────────────────────────────────┐
...
| 操作类型 | 路径 | 说明 |
|----------|------|------|
| 列表查询 / 详情 / Dashboard 聚合 | 前端 SDK 直连数据库 | 利用 CloudBase 安全规则做读权限；高频读直连，性能好 |
| 写操作（入库 / 变动 / 审批 / 借还 / 通知发布 / 账号绑定） | 前端调用云函数 | 云函数集中鉴权、参数校验、写 `ams_asset_log`、必要时事务 |
| 图片 / 签名上传 | 前端 SDK 直传云存储 | 上传成功后将 `fileID` 传给云函数落库 |

> 教师端因小程序网络限制，建议默认全部走云函数；如有性能需要再放开只读集合。

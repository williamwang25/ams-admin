...
> **作用**：定义云函数清单、入参出参约定，以及 SDK 直连白名单。
> **读者**：后端 agent、前端调用方。
...
> 通过环境变量传入 `INIT_SECRET`，调用时携带；初始化后建议禁用。
...
教师端 / 管理端可使用 CloudBase JS SDK 直接读取以下集合（**不允许 SDK 写**，由云数据库安全规则强制）：
...
> 安全规则的具体表达式由后端 agent 在 CloudBase 控制台配置；本文档定义意图。

...
> **AMS 项目上下文（必先读）**
>
> 本工作区是 **AMS 资产管理系统** 的管理端。在动手之前必须先读：
> 1. `AMS.md`（本工作区精简手册：项目身份、必读文档、工程铁律）
> 2. `docs/README.md` → `docs/03-data-model.md` → `docs/04-api-spec.md` → `docs/05-admin-features.md`
> 3. `.memory/PROGRESS.md`（当前进度）
>
> `docs/` / `.memory/` / `data/` 通过 junction 挂载自父仓库，写入即写入根仓库单一事实源。
> 业务约定（字段、状态、读写路径）以 `docs/` 为准；下面 CloudBase 模板规则提供实现细节。
